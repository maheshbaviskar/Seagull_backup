...
sed -i "s/avp\ name\=\"Rating-Group\"\ value\=\"$flag5/avp\ name\=\"Rating-Group\"\ value\=\"$GPP/g" scenario-oca-pcef-data.xml
sed -i "s/avp\ name\=\"Event-Timestamp\"\ value\=\"$flag2/avp\ name\=\"Event-Timestamp\"\ value\=\"$TIMESTAMP/g" scenario-oca-pcef-data.xml
sed -i "s/avp\ name\=\"CC-Total-Octets\"\ value\=\"$flag3/avp\ name\=\"CC-Total-Octets\"\ value\=\"$SNC/g" scenario-oca-pcef-data.xml
#sed -i "s/avp\ name\=\"3GPP-User-Location-Info\"\ value\=\"$flag4/avp\ name\=\"3GPP-User-Location-Info\"\ value\=\"$NC/g" scenario-oca-pcef-data.xml
...
echo "$PURPLE pls see Notification's generated in below Event xml's  $NONE\n $fname1"
echo "\n"
